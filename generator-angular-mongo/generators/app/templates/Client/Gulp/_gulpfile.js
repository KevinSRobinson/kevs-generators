...
gulp.task('test-e2e',  function(){
    gulp.src(["./src/e2eTests/*.js"])
    .pipe(protractor({
        configFile: "protractor.config.js",
        args: ['--baseUrl', 'http://localhost:8001/']
    }))
    .on('error', function(e) {
        utils.log(e);
    })
...
gulp.task("inject-js-karma-conf", function () {
    return gulp
        ...
        .pipe($.inject(gulp.src('./src/Client/**/*.spec.js'), {
            starttag: "// spec:js",
            endtag: "// endinject",
            transform: function (filepath, file, i, length) {
...
gulp.task('serve-build', ['embedTemplates', 'optimize', 'bump'], function(){
   myGulp_BrowserSync.serve(false, config.port);
});

gulp.task('serve-dev', ['wiredep', 'inject', 'embedTemplates'], function(){
   myGulp_BrowserSync.serve(true, config.port);
});
